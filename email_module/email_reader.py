...
try:
    ...
    import os
    import platform
    from utils.email_cleaner import is_terms_and_conditions_file, filter_terms_and_conditions_content
    PDF_SUPPORT = True
                    ...
                    if filename:
                        ...
                        if payload:
                            ...
                            if content_type == 'application/pdf' or filename.lower().endswith('.pdf'):
                                ...
                                if pdf_text:
                                    # Check if this is a T&C document and truncate if so
                                    if is_terms_and_conditions_file(filename):
                                        logger.info(f"Detected T&C document: {filename} ({len(pdf_text)} chars) - truncating to save tokens")
                                        pdf_text = filter_terms_and_conditions_content(pdf_text, max_length=500)
                                    combined_text += f"\n\n=== ATTACHMENT: {filename} ===\n{pdf_text}\n"

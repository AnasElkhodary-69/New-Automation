...
class RAGEmailSystem:
    ...
    def _initialize_modules(self):
        ...
        # Initialize email notifier (for processing notifications)
        self.email_notifier = EmailNotifier()

        # Initialize Telegram notifier (for Telegram notifications)
        ...
        try:
            ...
            for idx, email in enumerate(unread_emails, 1):
                ...
                if result.get('success'):
                    ...
                    self.processor.step_logger.create_summary_file(email, result)

                    # Step 6: Send email notification
                    self.email_notifier.send_processing_notification(email, result)

                    # Step 7: Send Telegram notification and store order result
                    order_id = f"ORDER_{idx}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

                    # Send Telegram notification
                    telegram_response = self.telegram_notifier.send_order_notification(
                        email=email,
                        result=result,
                        order_id=order_id
                    )

                    # Store order result for feedback tracking
                    if telegram_response:
                        self.feedback_storage.store_order_result(
                            order_id=order_id,
                            email=email,
                            result=result,
                            telegram_message_id=telegram_response.get('message_id')
                        )
                        logger.info(f"Order {order_id} stored for feedback tracking")
    ...
    def _log_response(self, original_email: Dict, processing_result: Dict):
        ...
        if order_created:
            if order_created.get('created'):
                ...
                logger.info(f"    Amount Total: EUR {order_created.get('amount_total', 0):.2f}")
